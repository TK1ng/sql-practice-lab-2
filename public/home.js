...
function getPendingAppointments() {
    axios.get('http://localhost:4400/pending')
        .then(res => {
...
function getUpcomingAppointments() {
    axios.get('http://localhost:4400/upcoming')
        .then(res => {
...
function getPastAppointments() {
    axios.get('http://localhost:4400/appt')
        .then(res => {
            for (let i = 0; i < res.data.length; i += 2) {
                const appt = res.data[i]
                const nextAppt = res.data[i + 1] || null
...
function markApproved(id) {
    ...
    axios.put('http://localhost:4400/approve', { apptId: id })
...
function markCompleted(id) {
    ...
    axios.put('http://localhost:4400/complete', { apptId: id })
